...
#define AAKERNEL_SIZE 6			/* Supersample size */
...
int Application5::Initialize()
{
	...
	for (int i = 0; i < AAKERNEL_SIZE + 1; i++) {
		...
		*/
		...
		if (i == AAKERNEL_SIZE) {
			valueListShift[X] = (GzPointer)&offset;
			valueListShift[Y] = (GzPointer)&offset;
		}
		else {
			valueListShift[X] = (GzPointer)&(AAFilter[i][X]);
			valueListShift[Y] = (GzPointer)&(AAFilter[i][Y]);
		}
	...
	*/
	...
	for (int i = 0; i < m_nWidth * m_nHeight; i++) {
		...
		for (int j = 0; j < AAKERNEL_SIZE; j++) {
			// Color C = pixelbuffer.color * weight
			redSum += (float)m_pRender[j]->pixelbuffer[i].red * AAFilter[j][2];
			greenSum += (float)m_pRender[j]->pixelbuffer[i].green * AAFilter[j][2];
			blueSum += (float)m_pRender[j]->pixelbuffer[i].blue * AAFilter[j][2];
		}

		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i].red = (short)(redSum + 0.5);
		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i].green = (short)(greenSum + 0.5);
		m_pRender[AAKERNEL_SIZE]->pixelbuffer[i].blue = (short)(blueSum + 0.5);
